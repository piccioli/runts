...
def _fmt(value, decimals: int = 2) -> str:
    ...
    try:
        return (
            f"{float(value):,.{decimals}f}".replace(",", "X")
            .replace(".", ",")
            .replace("X", ".")
        )
...
def _build_allegati_section(allegati, styles) -> list:
    ...
    header_style = ParagraphStyle(
        "hdr", parent=styles["Normal"], fontSize=8, textColor=colors.grey
    )
    ...
    for att in allegati:
        try:
            ...
            link_cell = Paragraph(
                f'<link href="{url}">↗</link>' if url else "—", cell_style
            )
...
def _build_bilanci_section(bilanci, styles) -> list:
    ...
    for b in bilanci:
        try:
            ...
            ris_color = (
                colors.red
                if risultato is not None and float(risultato) < 0
                else colors.black
            )
            ris_style = ParagraphStyle(
                "ris",
...
def _build_cariche_section(cariche, styles) -> list:
    if not cariche:
        return []
    cell_style = ParagraphStyle("cell", parent=styles["Normal"], fontSize=8)
    hist_style = ParagraphStyle(
        "hist", parent=styles["Normal"], fontSize=8, textColor=colors.grey
    )
    header_style = ParagraphStyle(
        "hdr", parent=styles["Normal"], fontSize=8, textColor=colors.grey
    )
...
def build_ente_pdf(
    ente_row: sqlite3.Row, *, allegati=None, bilanci=None, cariche=None
) -> bytes:
    ...
    story = [
        ...
        Paragraph(
            f"Codice fiscale: {codice_fiscale}" if codice_fiscale else "", sub_style
        ),
    ]
    ...
    try:
        ...
        for page in content_reader.pages:
            ...
            carta_copy.merge_page(page)  # type: ignore[attr-defined]
            writer.add_page(carta_copy)  # type: ignore[arg-type]
