...
_SUBSECTIONS_URL = (
    "https://www.cai.it/wp-json/cai-section/v2/sections/{code}/sub-sections-list"
)
...
def _normalize_section(raw: dict, regione: str) -> dict:
    ...
    return {
        ...
        "cai_indirizzo_sede": json.dumps(office_addr, ensure_ascii=False)
        if office_addr
        else None,
        "cai_indirizzo_postale": json.dumps(postal_addr, ensure_ascii=False)
        if postal_addr
        else None,
...
def _normalize_subsection(raw: dict, codice_sezione: str) -> dict:
    ...
    return {
        ...
        "cai_avvisi": raw.get("notice") or None,
        "cai_anno_fondazione": raw.get("foundationYear") or None,
        "cai_soci": raw.get("currentMemberships")
        or raw.get("lastyearMembershipsCount")
        or None,
        "cai_lat": raw.get("latitude") or None,
        "cai_lon": raw.get("longitude") or None,
        "cai_scraped_at": datetime.now(timezone.utc).isoformat(),
...
def fetch_subsections(codice_sezione: str) -> list[dict]:
    ...
    if not isinstance(data, list):
        logger.warning(
            "Unexpected response type for subsections of %s: %s",
            codice_sezione,
            type(data).__name__,
        )
        return []
...
def fetch_regional_groups() -> list[dict]:
    ...
    if not isinstance(data, list):
        logger.error(
            "Unexpected response type for regional groups: %s", type(data).__name__
        )
        return []
        ...
        results.append(
            {
                ...
                "gr_indirizzo_sede": json.dumps(office_addr, ensure_ascii=False)
                if office_addr
                else None,
                "gr_indirizzo_postale": json.dumps(postal_addr, ensure_ascii=False)
                if postal_addr
                else None,
