...
def test_upsert_preserves_lat_lon_on_rerun(mem_db):
    ...
    # Verify stored
    row = mem_db.execute(
        "SELECT lat, lon FROM enti WHERE id_runts='TEST001'"
    ).fetchone()
    ...
    row = mem_db.execute(
        "SELECT lat, lon, denominazione FROM enti WHERE id_runts='TEST001'"
    ).fetchone()
...
def test_upsert_preserves_lat_lon_when_none_in_dict(mem_db):
    ...
    upsert_ente(
        mem_db, {"id_runts": "TEST002", "denominazione": "X", "lat": 45.0, "lon": 9.0}
    )

    upsert_ente(
        mem_db,
        {"id_runts": "TEST002", "denominazione": "X updated", "lat": None, "lon": None},
    )

    row = mem_db.execute(
        "SELECT lat, lon FROM enti WHERE id_runts='TEST002'"
    ).fetchone()
...
def test_upsert_sets_lat_lon_on_first_insert(mem_db):
    ...
    upsert_ente(
        mem_db, {"id_runts": "TEST003", "denominazione": "Y", "lat": 41.9, "lon": 12.5}
    )
    row = mem_db.execute(
        "SELECT lat, lon FROM enti WHERE id_runts='TEST003'"
    ).fetchone()
